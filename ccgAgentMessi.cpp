...
int ccgAgentMessi(Hand hand, Card lastBowledCard, bool isBatting, const MatchState &match)
{
   Card card;
   ...
   else // is bowling
   {
      ...
      for (whichCard = 0; whichCard < numCardsPerHand; whichCard += 1)
      {
         ...
         if (card.getNumber() < 5)  // if the card is a low number, look to see if you have another card with the same suit but a higher number
         {                          // if you do, play the low card
...
/*
 ...
 For iteration 2 I decided to mainly focus on my bowling strategy because that's where I was lacking according to the results.
 First i decided to clean up the numbers. Originally the quality numbers were just some arbitrary numbers with no pattern.
 I decided to come up with a number for the ideal card and then adjust the quality for worse cards. I finalized those numbers
 by playing my agent against itself just with different numbers. I then took the numbers from the one with the lower R/W and B/W
 and put them in the final agent. The biggest advancement I made was when I started playing low cards if I had a higher card of
 the same suit in my hand. This sacrifices some runs, but ultimately it works as a bait. I get the batter to use their big cards
 on the small numbers and then play the higher number, sometimes they wont have a high enough card to cover it so I take a wicket.
 When I put this strategy in my B/W dropped 4 points. I then used the same strategy of playing the agent against itself and using
 hill climbing to find the right quality number for that strategy.
